...
bank = read.csv("~/Documents/Northeastern University/ALY 6010/dataset/bank dataset.csv")
bank
...
model2 <- lm(age ~ balance, data = BalanceSubset) #building a model for these two variables in BalanceSubset subset
model2
...
#Q-Q Plot for duration and day
qqnorm(DaysSubset$duration, pch = 1, frame = FALSE)
qqline(DaysSubset$duration, col = "green", lwd = 2)
...
model2 <- lm(day ~ duration, data = DurationSubset) #building a model for these two variables in DurationSubset subset
model2
...
shapiro.test(CampaignSubset$balance)

#Q-Q Plot for campaign equal to 28 days
...
model2 <- lm(balance ~ campaign, data = CampaignSubset) #building a model for these two variables in CampaignSubset subset
model2
